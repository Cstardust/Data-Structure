 ...
 * 常量轮值表,密钥扩展用
 */
unsigned int Rcon[] = {0, 0x01000000, 0x02000000,
...
//  循环左移1个bytes
int shiftLeftLoopOneByte(int num)
{
    return (num & 0x00ffffff) << 8 | (num & 0xff000000) >> 24; //  "+" 就不行 离谱
}
...
//  根据 int的每个字节 ---得出--> 从SBOX中取出相应位置的数(1 bytes)  --> 存入int的相应位置
//   对int的四个字节都这么做
//  int的每一个bytes都需要当作坐标去S盒中找数据 并将数据放到相应byte的位置上!
//  只有当int的每一个bytes都需要当作索引时 才可以用这个函数！！
int getIntNumFromSBox(int x)
{
    int res = 0;
    for (int i = 3; i >= 0; --i)
    {
        int t = (x >> 8 * i) & 0x000000ff;
        // printf("t = %d\t",t);
        // printf("get = %d\n",getNumFromSBox(t));
        // printf("get & = %d\n",(getNumFromSBox(t) & 0x000000ff));
        // printf("i want %d\n",(0 | (getNumFromSBox(t) & 0x000000ff)) << (8*i));
        res = res | (getNumFromSBox(t) & 0x000000ff) << (8 * i); //  <<24 | <<16 | <<8 | <<0
    }
    return res;
}
...
int getIntNumFromS1Box(int x)
{
    int res = 0;
    for (int i = 3; i >= 0; --i)
    {
        int t = (x >> 8 * i) & 0x000000ff;
        res = res | ((getNumFromS1Box(t) & 0x000000ff) << (8 * i)); //  <<6 | <<4 | <<2 | << 0
    }
    return res;
}
...
void subBytes(int array[4][4])
{
    ...
    for (int i = 0; i < 4; ++i)
    {
        for (int j = 0; j < 4; ++j)
        {
            ...
            //  不能用辅助函数getInt 因为即便array[i][j]只有最低一个bytes有有效数据
            //  但是 其高位的0 也会被当作坐标 去S盒中找到相应位置的数 加入 res ，最后返回
            //  因此 绝对不可！
            //  只有当int的每一个bytes都需要当作坐标去S盒中找数据 并将数据放到相应byte的位置上时
            //  才可以用getIntNumFromSBox
            // array[i][j] = getIntNumFromSBox(array[i][j]);
            // printf("arr = %d\n",array[i][j]);
            // printf("intNum = %d \n",getIntNumFromSBox(array[i][j]));
            // printf("num = %d \n",getNumFromSBox(array[i][j]));
        }
    }
}
...
{
    //  array左乘一个矩阵，结果存入array
    int backup[4][4];
    memcpy(backup, array, 64);
...
void addRoundKey(int array[4][4], int round)
{
    ...
    {
        int w_arr[4];
        splitIntToArray(w[w_base + i], w_arr); //  获取那一列key 如 k1 k2 k3 k4
        for (int j = 0; j < 4; ++j) //  行
        {
            array[j][i] = array[j][i] ^ w_arr[j];
        }
    }
}

void util_bits_int(int x)
{
    for (int i = 31; i >= 0; --i)
    {
        if (!((i + 1) % 8))
...
int T(int num, int round)
{
    ...
    //  1. 字循环
    //  循环左移 1 byte
    num = shiftLeftLoopOneByte(num);
    //  2. 根据num 将int的逐个字节进行S盒代换 并放入s_num每个字节相应位置
    int s_num = getIntNumFromSBox(num);
    //  3. ^ ronj
    int res = s_num ^ Rcon[round];
    return res;
}
...
void extendKey(char *key)
{
    ...
    while (idx < 44)
    {
        if (!(idx % 4))
        {
            w[idx] = w[idx - 4] ^ T(w[idx - 1], idx / 4);
        }
        else
        {
            w[idx] = w[idx - 4] ^ w[idx - 1];
        }
        ++idx;
    }
}
...
void aes(char *p, int plen, char *key)
{
    ...
    for (k = 0; k < plen; k += 16)
    {
        ...
        addRoundKey(pArray, 0); //  0轮 一开始的轮密钥加
...
void deAes(char *c, int clen, char *key)
{
    ...
    for (k = 0; k < clen; k += 16)
    {
        ...
        convertArrayToStr(cArray, c + k); //  output to c
    }
}
/**
 *读取字符串
